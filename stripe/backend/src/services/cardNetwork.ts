 ...
 */
export async function capture({ authCode, amount: _amount, currency: _currency }: CaptureParams): Promise<CaptureResult> {
  const startTime = process.hrtime();
 ...
 * Also protected by circuit breaker
 */
export async function refund({ authCode: _authCode, amount, currency }: RefundParams): Promise<RefundResult> {
  const startTime = process.hrtime();
