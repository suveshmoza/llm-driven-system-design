...
export interface GpsPointsRetention {
  fullResolutionDays: number;
  downsampleFactor: number;
  archivalSchedule: string;
}

export interface SegmentEffortsRetention {
...
export interface ActivityFeedsRetention {
  ttlDays: number;
  maxFeedSize: number;
}

export interface SessionsRetention {
  ttlHours: number;
}

export interface LeaderboardsRetention {
  rebuildOnStartup: boolean;
  rebuildSchedule: string;
}

export interface RetentionConfig {
  gpsPoints: GpsPointsRetention;
  segmentEfforts: SegmentEffortsRetention;
  activityFeeds: ActivityFeedsRetention;
  sessions: SessionsRetention;
  leaderboards: LeaderboardsRetention;
}
...
export const archival: ArchivalConfig = {
...
export interface SegmentMatchingAlerts {
  durationWarnMs: number;
  durationCriticalMs: number;
  maxCandidateSegments: number;
}

export interface ActivityUploadAlerts {
  processingTimeWarnMs: number;
  processingTimeCriticalMs: number;
  maxGpsPoints: number;
...
export interface DatabaseAlerts {
  queryTimeWarnMs: number;
  queryTimeCriticalMs: number;
  connectionPoolLowThreshold: number;
  connectionPoolHighUtilization: number;
}

export interface RedisAlerts {
  operationTimeWarnMs: number;
  operationTimeCriticalMs: number;
}

export interface StorageAlerts {
  gpsPointsTableMaxMB: number;
  activitiesTableMaxMB: number;
  segmentEffortsTableMaxMB: number;
}

export interface CacheAlerts {
  feedHitRateTarget: number;
  leaderboardHitRateTarget: number;
  userProfileHitRateTarget: number;
}

export interface ApiAlerts {
  responseTimeP50TargetMs: number;
  responseTimeP95TargetMs: number;
  responseTimeP99TargetMs: number;
}

export interface AlertsConfig {
  queue: QueueAlerts;
  segmentMatching: SegmentMatchingAlerts;
  activityUpload: ActivityUploadAlerts;
...
export const idempotency: IdempotencyConfig = {
...
export const healthCheck: HealthCheckConfig = {
